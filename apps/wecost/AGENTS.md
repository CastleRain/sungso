...
앱을 `apps/wecost/`, 재무 공통 코드를 `shared/finance/`로 분리하고 기존 공개 URL을 유지했다. 재무 계산·목표가격 연결과 모듈 동일성 검사를 포함한 전체 1,403개 검사를 통과했다. 1440px·390px에서 네 탭 전환·배치와 가로 넘침 없음을 확인했다. 운영 재무 데이터에는 테스트 값을 쓰지 않았으며 공개 배포 확인은 진행 중이다. [전체 검증 근거](../../docs/restructure-verification.md)를 참고한다.

**다음:** 공개 네 탭과 HomeHunt 목표가격 연결을 확인한다.
