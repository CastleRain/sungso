...
회원 전용 신혼여행의 현재 일정·지도·호텔 후보·여행 비용·출발 준비와 변경 이력을 함께 관리한다.

- `navigation.mjs`: 전체 일정·숙소·항공/크루즈·리조트/이동·관광/식사·여행 비용·출발 준비·변경 기록의 여덟 화면.
- `calendar.mjs`와 `calendar.css`: 여행 기간/해당 월 전체 달력, 이동일 출발·도착 국가, 선택 날짜·지도 연결.
...
- `shared/travel/`: 인증 후 보호된 여행 기준 데이터·검증·Firestore 저장과 구독. Honeymoon의 현재 여행 요약도 같은 저장소를 읽는다.
