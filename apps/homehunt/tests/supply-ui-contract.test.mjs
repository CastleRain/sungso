...
test('scheduled collection explicitly dispatches the artifact deployment after data changes', async () => {
  ...
  assert.match(workflow, /gh workflow run deploy-pages\.yml --ref "\$\{GITHUB_REF_NAME\}"/);
