...
import { createRenderServer } from '../../../services/homehunt/render/node-server.mjs';
import { readRenderConfiguration } from '../../../services/homehunt/render/config.mjs';
import { createConfiguredHomehuntApi } from '../../../services/homehunt/server/create-api.mjs';
...
test('the Node adapter preserves member authorization and private API CORS', async t => {
  ...
  assert.equal(data.version, '2.11.0');
...
test('Render blueprint explicitly selects free Node hosting and server-managed secret inputs', async () => {
  ...
  const legacy = await readFile(new URL('../../../services/homehunt/cloud/index.mjs', import.meta.url), 'utf8');
