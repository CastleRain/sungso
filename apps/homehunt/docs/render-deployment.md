...
| Node | `22` (`services/homehunt/render/package.json`은 `22.x`) |
...
6. **인증과 실제 데이터 검증을 마친 주소만** `apps/homehunt/js/config.js`의 `CLOUD_API_BASE_URL`에 `<실제 Render 서비스 주소>/api` 형태로 입력한다. 허용 Origin은 공개 `https://castlerain.github.io`와 개발용 localhost로 제한한다. 토큰을 보내는 대상이 이 검증된 API 주소인지 확인한다.
