...
> 소스 경로는 2026-09-13 구조 정리에 맞췄다. 기능·조사·검증 날짜는 본문의 해당 버전 기록이며, 현재 실행·배포는 [개발 안내](../../../docs/development.md)와 [배포 안내](../../../docs/deployment.md)를 따른다.

## 2026-09-09 구성 확인 기록

**2026-09-09 공개 연결 완료:** `f269d0e`의 [master Pages 배포](https://github.com/CastleRain/sungso/actions/runs/34244603656)와 같은 소스의 **Render Free·Singapore·Node 22 검색 서버**가 정상 배포됐다. Firebase Spark 로그인·DB와 기존 개인 백업은 유지한다. API는 [`https://sungso-homehunt-api.onrender.com/api`](https://sungso-homehunt-api.onrender.com/api)다. 공개 UI 4.13.1의 실제 로그인·단지 검색·최근 1년 실거래 59건 표시, 회원 health 200·무로그인 401, 가격 작업 완료·재조회와 공식 주차·난방·승강기 응답을 확인했다. [Render 배포 절차](render-deployment.md)와 [Firestore 무료 운영](firestore-free-operation.md)이 현재 운영 기준이다.

현재 한국 날짜의 통근 전환 표식을 적용하고 지난 날짜 사용량은 이월하지 않았다. 공식 역 경로 1회가 정상 응답했으며 실제 Kakao 사용량·Firestore 장부·화면 잔여량이 1회 사용·999회 남음으로 일치했다. Render는 Kakao 일일 1,000회·TMAP 0회, 로컬은 신규 통근 0회로 유지하므로 통근 확인은 공개 화면에서 사용한다. NAVER 장소 검색도 실제 200·5개 결과를 확인했으며 추가 통근 호출은 없었다. 기존 UID 개인 백업은 덮어쓰지 않았고 예산·회사 조건을 유지했다. 복원된 회사 주소는 필요한 위치 재확인 후 사용한다. 일반·역 자료 1,097개·Firestore Emulator 보안·정리 20개 검사와 Render 빌드를 통과했다. 전체 후보의 회사 통근을 일괄 검증한 것은 아니다.

공통 서버는 인증된 K-apt 주차·난방·승강기 조회 `/kapt/complex`와 공식 원자료 Firestore 캐시(목록 7일·기본/상세 1일)를 제공하도록 준비했다. 클라우드 API 계약은 2.9.0이다. UI는 온라인 검색을 연결해도 기존 `homehunt_user_snapshots/{uid}` 개인 백업을 계속 사용하며, 가구 백업은 `snapshotTransport: 'api'`를 명시할 때만 사용한다. 로그인 전 시설 대기열은 호출하지 않고 로그인 후 이어가며 사용자의 일시정지를 보존한다.
...
2026-09-08 확인 시 `sungso-358cb`는 Spark 무료 플랜이며 Firestore 위치는 서울(`asia-northeast3`)이다. Google 로그인과 실제 Pages·localhost 도메인 설정을 적용했다. Functions는 배포하지 않았으며, Render 배포를 위해 Blaze나 별도 유료 DB로 전환할 필요는 없다. 아래 Functions 절차는 나중에 선택할 수 있는 대안으로 남긴다. Functions를 선택하면 Blaze 전환이 필요하고 요금제는 자동 변경하지 않는다. [Firebase 설명](https://firebase.google.com/docs/functions/faq-and-troubleshooting)

**개인 백업 배포 이력:** 2026-09-08 KST, 릴리스 `5978806`의 [Pages 배포](https://github.com/CastleRain/sungso/actions/runs/34151237385)가 성공했다. 당시 공개 4.4.0 HTML·클라우드 모듈·CSS는 HTTP 200이며 실제 화면에서 지도·저장 UI·온라인 검색 미배포 표시를 확인했다. 로컬 화면의 실제 Google 로그인·새로고침 유지·비공개 백업과 저장 전 교체 취소를 검증했고, 관리자 조회로 회사 조건 보존·경로/점수/공급자 좌표 미저장을 대조했다. 일반 테스트 694개, 실제 Firestore Emulator 보안 테스트 16개를 통과했으며 운영 Firebase의 무로그인 개인 기록·회원·경로 캐시 읽기는 모두 403이었다. 해당 작업의 실제 통근 원호출은 0회다. 최신 화면 배포 이력은 [배포 문서](deployment.md)를 따른다.

최종 문서 커밋 `805e5b3`의 Pages 배포도 성공했다. 공개 사이트에서 Google 로그인 후 같은 Firebase 백업을 실제 불러와 회사 조건·예산 출처 보존과 위치 재확인 안내를 확인했다. 로컬 브라우저의 기존 조건은 유지했다.
...
| 공식 국토부 월 자료 | 배포한 Render 검색 서버의 Firestore 공개자료 캐시. 공개 화면의 실제 이력·작업 재조회 확인 |
| TMAP 경로 | 구현상 조회시각부터 8시간 재사용. 현재 운영 활성화 대상은 아니며 같은 출발·도착·수단·출발일시·옵션·스키마가 일치해야 함 |
...
## Firebase Functions 배포 (대안)

현재 선택은 [Render Free](render-deployment.md)다. 아래 명령은 사용자가 나중에 Firebase Functions와 Blaze 전환을 선택했을 때만 실행한다. `services/homehunt/cloud`는 별도 Firebase Functions 코드베이스(`homehunt`)이며 Render와 HTTP·인증·DB 코어를 공유한다. Node 22, 서울 리전, 최소 인스턴스 0·최대 2, 인스턴스당 동시 요청 1, 512MiB, 요청 제한 540초로 설정했다. 최대 인스턴스와 호출 한도는 과금 상한이 아니므로 결제 전환 시 예산 알림도 설정한다.

```powershell
# 저장소 루트에서 실행
npm ci --prefix services/homehunt/cloud
npm run build --prefix services/homehunt/cloud
node --test apps/homehunt/tests/*.test.mjs
node --check apps/homehunt/js/app.js
git diff --check

# 사용자가 Blaze 전환을 완료한 뒤 실행한다.
# Secret JSON은 Git 제외 파일로 준비하며 값은 터미널에 출력하지 않는다.
firebase functions:secrets:set HOMEHUNT_PROVIDER_CONFIG --project sungso-358cb --data-file services/homehunt/.local/cloud-provider-config.json
...
5. 현재 Render·Spark 구성은 **관리형 TTL 정책 없이** [제한된 자체 정리](firestore-free-operation.md)를 사용한다. 나중에 Blaze에서 관리형 TTL을 선택하려면 비용·컬렉션 범위·자체 정리와의 관계를 별도로 검토한다. TTL 삭제는 즉시 실행되지 않으므로 코드의 만료 검사는 계속 필요하며 당일 통근 호출 장부를 삭제하는 정책을 만들지 않는다. [Firestore TTL](https://firebase.google.com/docs/firestore/ttl)
6. 기존 Pages 절차로 정적 화면을 배포하고 휴대폰에서 로그인·검색·저장·복원을 확인한다.

가격 검색은 Firestore 작업으로 저장하고 `/advance` 요청마다 최대 8개 월 자료를 처리한다. 모든 처리·체크포인트를 응답 전에 기다리며 작업은 24시간 유효하다. 창을 닫았을 때 무기한 백그라운드 실행하는 방식은 아니고, 작업 ID로 다시 요청하면 이어간다. 하나의 월은 40초 제한을 적용하고 작업 임대·중복 갱신 방지·취소를 지원한다. 일부 월 실패는 정상 자료와 검증된 이전 자료를 유지하고 잠정 가격·미완료 월로 구분한다.
...
| 선택 | 가능한 범위 | 추가 조건 |
|---|---|---|
| Pages + Firebase Spark | 정적 화면 + Google 로그인 + 계정별 개인 기록 저장 | 현재 사용 중 |
| Pages + Render Free + Firebase Spark | 위 기능 + PC 없이 가격 검색·통근·공식 시설·자료 캐시 | 현재 운영. 공개 UI·인증·실제 가격·시설·통근 1회 검증. 첫 접속 대기·공유 750시간·외부 통신·DB 무료 한도 적용 |
