...
- 매거진·필름·온실·추억장·별자리·봉투·산책길의 **스크롤 연출**이 기본으로 켜진다. 목록의 일곱 바로가기나 **스크롤로 펼쳐보기**에서 모바일 보기로 바로 연다. 사진 확대·필름 셔터·꽃 모양 열림·종이 넘김·별빛 깊이 이동·편지 들림·가로 산책처럼 서로 다른 전환을 제공한다. 한 화면을 고정하고 문서 스크롤 위치에서 연출을 계산하므로 위로 올리면 같은 순간으로 돌아간다.
...
- `scroll-designs.mjs`·`scroll-motion*.mjs`와 대응 CSS: 기존 일곱 ID의 스크롤 연출 등록과 장면별 순수 계산. `scroll-library.mjs`·`css/scroll-library.css`는 목록 바로가기를 제공한다. 중첩된 체험 컨테이너를 유지하며 각 본문 장면만 화면에 배치한다. [확장 검증](../../docs/invitation-scroll-editions-verification.md)을 따른다.
