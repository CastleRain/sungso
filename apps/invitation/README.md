...
성우·소희가 총 23가지 예시를 끝까지 내려보며 비교하고, 각자의 찜과 공동 디자인 선택을 저장하는 앱이다. 기존 20종에 종이 극장·이야기의 집·리본 편지 3종을 더했다. 하객에게 전달할 실제 청첩장 제작·공개 기능은 별도 작업이다.
...
| ID | 디자인 | 본문 체험 |
|---|---|---|
| `paper-theater` | 종이 속 작은 결혼식 | 겹친 종이 무대의 창 열림·장면 선택·약속 무대 |
| `memory-house` | 우리라는 집에 초대합니다 | 집의 외벽 열림·거실/사진방/정원 선택·정원 문 열림 |
| `ribbon` | 한 줄로 이어진 우리 | 매듭 풀림·세 면의 카드 펼침·이야기 선택·약속 리본 |
...
- 모든 23종의 **스크롤 연출**이 기본으로 켜진다. 별도의 스크롤 소개 선반과 중복 링크는 없애고, 한 목록의 **모바일로 보기**로 열도록 정리했다. 기존 일곱 연출과 함께 음반 회전·전시실 전환·축제 무대 상승을 지원하며, 새 세 예시는 장면 안의 종이 창·집의 벽·접힌 카드가 스크롤에 따라 펼쳐진다. 한 화면을 고정하고 문서 스크롤 위치에서 연출을 계산하므로 위로 올리면 같은 순간으로 돌아간다. **한 번에 펼쳐보기**를 켠 장면은 버튼을 다시 눌러 스크롤 연동으로 복귀한다.
