...
export function firestoreAdapter(sdk, db, { online = () => true, events = null, timeout = 15000 } = {}) {
  const ref = sdk.doc(db, ...DOCUMENT_PATH.split('/'));
  let callback, failure, unsubscribe, timer;
  const begin = () => {
    unsubscribe?.(); clearTimeout(timer);
    timer = setTimeout(() => failure?.(new Error('공동 저장 연결이 늦어지고 있어요. 예시는 계속 볼 수 있어요.')), timeout);
    ...
    }, error => { clearTimeout(timer); failure?.(error); });
  };
  const reconnect = () => begin();
  const offline = () => failure?.(new Error('인터넷 연결이 끊겼어요. 임시 설정은 유지되며, 연결 후 다시 저장할 수 있어요.'));
  return {
    subscribe(next, error) { callback = next; failure = error; begin(); events?.addEventListener('online', reconnect); events?.addEventListener('offline', offline); return () => { unsubscribe?.(); clearTimeout(timer); events?.removeEventListener('online', reconnect); events?.removeEventListener('offline', offline); }; },
    retry: begin,
    async transact(change) {
      ...
      await sdk.runTransaction(db, async tx => {
        ...
        if (change.type === 'selection') patch.updatedAt = sdk.serverTimestamp();
