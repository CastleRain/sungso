import { SIGNATURES, getTemplate } from './catalog.mjs?v=20260914-reference-samples';
...
export function signatureCollection() {
  return `<section class="signature-collection" aria-labelledby="signature-collection-title">
    ...
    <div class="signature-picks">${Object.entries(SIGNATURES).map(([id, edition], index) => `<a class="signature-pick pick-${id}" href="#preview/${id}" aria-label="${getTemplate(id).name} 시그니처 전체 미리보기"><span class="signature-pick-top"><span>${edition.label}</span><span>0${index + 1}</span></span><span class="signature-pick-art" aria-hidden="true">${id === 'envelope' ? '<i class="pick-envelope"><b>S & S</b></i>' : id === 'constellation' ? '<i class="pick-orbit"></i><b class="pick-stars">✦</b>' : '<i class="pick-ticket">SS 0518<span>TOGETHER, ALWAYS</span></i>'}</span><strong>${edition.title}</strong><span class="signature-pick-description">${edition.description}</span><span class="signature-pick-bottom">${getTemplate(id).name}<span aria-hidden="true">↗</span></span></a>`).join('')}</div>
  </section>`;
}
