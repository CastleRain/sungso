...
const sample = '<p class="pe-sample">가상의 사진·이야기·날짜·장소를 사용한 디자인 예시입니다.</p>';
const section = (key, className, body) => `<section class="pe-section ${className}" data-section="${key}">${body}</section>`;
...
const fixedDate = (title, detail) => `<div class="pe-date-numbers"><span>2030</span><strong>05<span> / </span>18</strong><span>SATURDAY · 2:00 PM</span></div><h2>${title}</h2><p>2030년 5월 18일 토요일 오후 2시<br>우리의 웨딩홀 · 가든홀</p><p class="pe-date-detail">${detail}</p><small class="pe-sample">날짜·시간·장소는 모두 예시입니다.</small>`;
const gallery = (parts, className, caption) => `<div class="pe-gallery ${className}">${label(caption)}${parts.gallery}</div>`;
const keepsakeEnd = (className, english, text) => `<footer class="invitation-ending pe-ending ${className}">${label('SUN​GWOO & SOHEE')}<p class="pe-ending-title">${english}</p><p>${text}</p><p class="pe-names">${names}</p><span class="pe-label">18 MAY 2030 · WITH YOU</span>${sample}</footer>`;
...
export function playfulCover(id, thumbnail = false) {
  ...
  switch (id) {
    case 'greenhouse': return `<div class="${classes}">${label('THE LITTLE GREENHOUSE')}<h2>Love<br>in <em>bloom.</em></h2><p class="pe-cover-korean">우리의 계절이 피어나는 곳</p><div class="pe-glasshouse"><div class="pe-glasshouse-photo">${photograph(0, '', !thumbnail)}</div><div class="pe-house-lines" aria-hidden="true"></div><span class="pe-leaf pe-leaf-left">${branch}</span><span class="pe-leaf pe-leaf-right">${branch}</span><span class="pe-greenhouse-tag">S & S<br><small>EST. 2030</small></span></div><p class="pe-names">${names}</p><p class="pe-cover-date">18 MAY 2030 · SATURDAY</p></div>`;
    case 'scrapbook': return `<div class="${classes}"><div class="pe-scrap-binding" aria-hidden="true"></div>${label('OUR LITTLE COLLECTION · VOL. 01')}<h2>Little things,<br><em>big love.</em></h2><div class="pe-scrap-photo"><span class="pe-tape" aria-hidden="true"></span>${photograph(1, '', !thumbnail)}<p>너와 나, 그리고 수많은 보통날.</p></div><span class="pe-heart-sticker" aria-hidden="true">LOVE<br>YOU!</span><div class="pe-cover-note"><span>TO. 소중한 당신</span><p>우리, 결혼해요.</p><span>성우 & 소희 · 2030. 05. 18</span></div>${thumbnail ? '' : toggle('sticker', '초대장에 하트 붙이기', '하트 떼고 다시 붙이기')}<span class="pe-added-sticker" aria-hidden="true">WITH<br>LOVE ♡</span></div>`;
    case 'festival': return `<div class="${classes}"><div class="pe-fest-masthead"><span>ONE DAY ONLY</span><span>18 MAY 2030</span></div><h2>LOVE<br><em>FEST.</em></h2><p class="pe-fest-lineup">SUN​GWOO × SOHEE</p><div class="pe-fest-stage">${photograph(0, '', !thumbnail)}<span class="pe-fest-rosette" aria-hidden="true">YOU'RE<br>INVITED</span><span class="pe-fest-banner">우리의 가장 다정한 축제</span></div><div class="pe-fest-bottom"><span>GARDEN STAGE<br>오후 2시 · 우리의 웨딩홀</span><strong>05.18</strong></div></div>`;
    case 'promenade': return `<div class="${classes}">${label('A WALK TO REMEMBER')}<h2>너와 나,<br><em>같은 걸음.</em></h2><p class="pe-cover-korean">오래도록 함께 걷고 싶은 길</p><div class="pe-walk-photo">${photograph(1, '', !thumbnail)}<svg viewBox="0 0 320 330" fill="none" aria-hidden="true"><path d="M17 293C119 345 308 289 253 213S66 210 114 120 266 99 298 21"/><circle cx="17" cy="293" r="7"/><circle cx="298" cy="21" r="7"/></svg><span class="pe-walk-note">START HERE<br>YOU + ME</span></div><div class="pe-walk-cover-end"><span class="pe-compass" aria-hidden="true">N<br>↟</span><p>성우 & 소희<br><span>2030. 05. 18 · 14:00</span></p><span aria-hidden="true">01 → ∞</span></div></div>`;
    default: return '';
  }
}
...
function scrapbook(selection, parts, enabled) {
  ...
  return `${parts.cover || playfulCover('scrapbook')}
    ...
    ${enabled('directions') ? section('directions', 'pe-scrap-directions', `${label('A LITTLE NOTE: HOW TO GET HERE')}<h2>잊지 않게 적어둘게요.</h2><div class="pe-address-note"><span>약속 장소</span><strong>우리의 웨딩홀 · 가든홀</strong><p>2030. 05. 18 토요일 · 오후 2시</p></div><div class="pe-scrap-transport"><article><span aria-hidden="true">↗</span><h3>지하철 메모</h3><p>예시역 2번 출구<br>정원길로 도보 약 5분</p></article><article><span aria-hidden="true">P</span><h3>주차 메모</h3><p>예시 주차장 이용<br>하객 2시간 무료</p></article></div><small class="pe-sample">장소와 교통 정보는 실제 안내가 아닌 예시입니다.</small>`) : ''}
...
function festival(selection, parts, enabled) {
  const stages = [['welcome', '13:30', 'WELCOME', '반가운 인사', '조금 먼저 도착해 서로의 안부를 나누어요. 가든홀 입구에서 환영 인사를 준비할게요.'], ['vows', '14:00', 'MAIN STAGE', '우리의 약속', '성우와 소희, 두 사람이 함께 만드는 첫 무대. 오래도록 서로의 가장 가까운 편이 되겠습니다.'], ['celebrate', '14:30', 'CELEBRATION', '함께 만드는 마지막 장면', '따뜻한 축하와 사진 한 장으로 그날의 기쁨을 나누어요. 소중한 얼굴들을 오래 기억하겠습니다.']];
  return `${parts.cover || playfulCover('festival')}
    ...
    ${section('date', 'pe-fest-program', `${label('THE WEDDING SETLIST')}<h2>ONE DAY.<br><em>ALL OUR LOVE.</em></h2><p class="pe-fest-date">2030년 5월 18일 토요일<br><strong>오후 2시 · 우리의 웨딩홀 가든홀</strong></p><div class="pe-stage-selector" role="group" aria-label="예시 식순 상세 선택">${stages.map(([id, time, english, title]) => `<button type="button" data-playful-action="stage" data-playful-value="${id}" aria-pressed="${id === 'welcome'}"><time>${time}</time><span>${english}<strong>${title}</strong></span><span aria-hidden="true">↗</span></button>`).join('')}</div><div class="pe-stage-detail" aria-live="polite">${stages.map(([id, time, english, title, text]) => panel('stage', id, 'welcome', `${label(`${time} · ${english}`)}<h3>${title}</h3><p>${text}</p>`)).join('')}</div><small class="pe-sample">식순·날짜·시간·장소는 모두 디자인 확인용 예시입니다.</small>`)}
...
function promenade(selection, parts, enabled) {
  ...
  return `${parts.cover || playfulCover('promenade')}
    ...
    ${section('date', 'pe-walk-destination', `${label('OUR NEXT DESTINATION')}<div class="pe-destination-sign"><span>다음 정거장</span><strong>우리의 결혼식</strong><span aria-hidden="true">→</span></div>${fixedDate('같은 길의 새로운 시작', '오월의 좋은 날, 이곳에서 만나요.')}`)}
