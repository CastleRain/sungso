...
const sample = '<p class="pe-sample">사진·이야기·장소는 디자인 예시입니다.</p>';
...
export function playfulCover(id, thumbnail = false) {
  ...
  switch (id) {
    ...
    case 'festival': return `<div class="${classes}"><div class="pe-fest-masthead"><span>ONE DAY ONLY</span><span>${WEDDING.enDate}</span></div><h2>LOVE<br><em>FEST.</em></h2><p class="pe-fest-lineup">SUN​GWOO × SOHEE</p><div class="pe-fest-stage">${photograph(0, '', !thumbnail)}<span class="pe-fest-rosette" aria-hidden="true">YOU'RE<br>INVITED</span><span class="pe-fest-banner">우리의 가장 다정한 축제</span></div><div class="pe-fest-bottom"><span>GARDEN STAGE<br>${WEDDING.koTime} · 우리의 웨딩홀</span><strong>${WEDDING.monthDay}</strong></div></div>`;
...
function festival(selection, parts, enabled) {
  const stages = [['welcome', '13:30', 'WELCOME', '반가운 인사', '조금 먼저 도착해 서로의 안부를 나누어요. 가든홀 입구에서 환영 인사를 준비할게요.'], ['vows', WEDDING.time24, 'MAIN STAGE', '우리의 약속', '성우와 소희, 두 사람이 함께 만드는 첫 무대. 오래도록 서로의 가장 가까운 편이 되겠습니다.'], ['celebrate', '14:30', 'CELEBRATION', '함께 만드는 마지막 장면', '따뜻한 축하와 사진 한 장으로 그날의 기쁨을 나누어요. 소중한 얼굴들을 오래 기억하겠습니다.']];
  return `${parts.cover || playfulCover('festival')}
    ...
    ${section('date', 'pe-fest-program', `${label('THE WEDDING SETLIST')}<h2>ONE DAY.<br><em>ALL OUR LOVE.</em></h2><p class="pe-fest-date">${WEDDING.koLong}<br><strong>${WEDDING.koTime} · 우리의 웨딩홀 가든홀</strong></p><div class="pe-stage-selector" role="group" aria-label="예시 식순 상세 선택">${stages.map(([id, time, english, title]) => `<button type="button" data-playful-action="stage" data-playful-value="${id}" aria-pressed="${id === 'welcome'}"><time>${time}</time><span>${english}<strong>${title}</strong></span><span aria-hidden="true">↗</span></button>`).join('')}</div><div class="pe-stage-detail" aria-live="polite">${stages.map(([id, time, english, title, text]) => panel('stage', id, 'welcome', `${label(`${time} · ${english}`)}<h3>${title}</h3><p>${text}</p>`)).join('')}</div><small class="pe-sample">예식 시작 외 식순과 시간·장소는 디자인 확인용 예시입니다.</small>`)}
