import { emptyProfile, MAX_GALLERY, validateProfile } from './profile-core.mjs?v=20260915-personal-invitation';
import { preparePhoto } from './profile-images.mjs?v=20260915-personal-invitation';
...
export function createProfileEditor({dialog, getState, save, onSaved = () => {}, isActive = () => true}) {
  ...
  const fieldNames = ['name','hall','address','mapUrl','transport','parking'];
  ...
  function load() {
    ...
    draft={coverId:value.coverId,galleryIds:[...value.galleryIds],venue:{...value.venue}};
  ...
  async function upload(event) {
    ...
    try {
      ...
      for(const {id,...photo} of prepared){photos[id]=photo;added[id]=photo;if(input.dataset.profileUpload==='cover')draft.coverId=id;else draft.galleryIds.push(id);}
  ...
  async function click(event) {
    ...
    if(draft.venue.name===''&&Object.values(draft.venue).some(Boolean)){status('예식장 이름을 함께 입력해주세요.',true);return;}
    const token=epoch;
    try{
      validateProfile({...emptyProfile(),...draft});
      actionsDisabled(true);status('두 사람의 설정을 저장하고 있어요…');
      const referenced=new Set([draft.coverId,...draft.galleryIds].filter(Boolean));
