...
import { salonCover, salonBody } from './reference-salon.mjs?v=20260915-wedding-date';
import { guestCover, guestBody } from './reference-toourguest.mjs?v=20260915-wedding-date';
import { PHOTOS, SIGNATURES, getTemplate } from './catalog.mjs?v=20260915-wedding-date';
import { escapeHtml as e } from './core.mjs?v=20260915-wedding-date';
import { paperCover } from './special-paper.mjs?v=20260915-wedding-date';
import { worldsCover } from './special-worlds.mjs?v=20260915-wedding-date';
import { signatureBody } from './signatures.mjs?v=20260915-wedding-date';
import { editionCover, editionBody } from './editions.mjs?v=20260915-wedding-date';
import { immersiveCover, immersiveBody } from './immersive-scenes.mjs?v=20260915-wedding-date';
...
export function cover(templateId, thumbnail = false) {
  const special = salonCover(templateId, thumbnail) || guestCover(templateId, thumbnail) || immersiveCover(templateId, thumbnail) || editionCover(templateId, thumbnail) || paperCover(templateId, thumbnail) || worldsCover(templateId, thumbnail);
  if (special) return special;
  const date = `<p class="cover-date">${WEDDING.dotted} <span>${WEDDING.weekdayEn}</span></p>`;
...
export function invitation(selection) {
  ...
  const calendar = `<div class="wedding-calendar" aria-label="${WEDDING.year}년 ${WEDDING.month}월 달력, ${WEDDING.day}일 ${WEDDING.weekday} 결혼식"><div class="calendar-week">${['일', '월', '화', '수', '목', '금', '토'].map(day => `<span>${day}</span>`).join('')}</div><div class="calendar-days">${weddingCalendarCells().map(day => day === null ? '<span aria-hidden="true"></span>' : `<span${day === WEDDING.day ? ` class="wedding-day" aria-label="${day}일 결혼식"` : ''}>${day}</span>`).join('')}</div></div>`;
  const parts = { cover: cover(selection.templateId), greeting: '', story: '', date: '', gallery: '', directions: '', accounts: '', rsvp: '', guestbook: '', ending: '' };
  parts.greeting = section('greeting', 'INVITATION', '같은 계절을 걸어갈 우리', `<div class="invitation-prose"><p>서로의 평범한 하루가<br>가장 소중한 순간이 되었습니다.</p><p>함께 웃고, 서로에게 기대며<br>우리만의 계절을 만들어가려 합니다.</p><p>그 시작에 소중한 당신을 초대합니다.<br>따뜻한 마음으로 함께해 주세요.</p></div><p class="signature">성우 <span>그리고</span> 소희</p>`);
  if (sections.story) parts.story = section('story', 'OUR LITTLE STORY', '함께 쌓아온 순간들', `<div class="story-list"><article><span>01</span><div><h3>처음 마주한 날</h3><p>낯선 두 사람이 서로의 하루를 궁금해하기 시작했어요.</p></div></article><article><span>02</span><div><h3>함께라서 좋은 날들</h3><p>작은 여행과 평범한 주말이 특별한 기억이 되었어요.</p></div></article><article><span>03</span><div><h3>앞으로의 모든 날</h3><p>이제 오래도록 같은 방향을 바라보려 해요.</p></div></article></div><small class="sample-caption">구성을 살펴보기 위한 예시 이야기입니다.</small>`);
  parts.date = section('date', 'SAVE OUR DATE', WEDDING.koDate, `<p class="wedding-time">${WEDDING.weekday} ${WEDDING.koTime}</p>${calendar}<p class="venue-name">우리의 웨딩홀 · 가든홀</p><small class="sample-caption">예식 날짜와 시간은 두 사람의 일정이며 장소는 예시입니다.</small>`);
  if (sections.gallery) parts.gallery = section('gallery', 'MOMENTS OF US', '우리의 순간들', `<div class="photo-gallery gallery-${galleryLayout}">${PHOTOS.map((photo, index) => `<button class="gallery-photo" type="button" data-action="photo" data-index="${index}" aria-label="예시 사진 ${index + 1} 크게 보기">${image(index)}${galleryLayout === 'filmstrip' ? `<span>0${index + 1} / OUR MEMORIES</span>` : ''}</button>`).join('')}</div><p class="gallery-help">${galleryLayout === 'grid' ? '사진을 누르면 크게 볼 수 있어요.' : '옆으로 넘기거나 사진을 눌러보세요.'}</p>`);
  if (sections.directions) parts.directions = section('directions', 'COME CELEBRATE', '오시는 길', `<p class="venue-name">우리의 웨딩홀</p><p>가든홀 · ${WEDDING.koTime}</p><div class="sample-map" role="img" aria-label="실제 장소가 아닌 예시 약도"><svg viewBox="0 0 340 200" aria-hidden="true"><rect width="340" height="200" fill="var(--soft)"/><path d="M-10 70H350M90-10V210M230-10V210M-10 158H350" stroke="var(--paper)" stroke-width="20"/><rect x="113" y="93" width="92" height="40" rx="7" fill="var(--accent)" opacity=".15"/><circle cx="165" cy="106" r="16" fill="var(--accent)"/><path d="M158 103q7-10 14 0q0 6-7 10q-7-4-7-10" fill="var(--paper)"/><text x="165" y="142" text-anchor="middle" fill="var(--ink)" font-size="11">우리의 웨딩홀</text><text x="42" y="58" fill="var(--ink)" font-size="10">예시역</text></svg><span>예시 약도</span></div><div class="sample-buttons"><span>네이버 지도</span><span>카카오맵</span><span>티맵</span></div><div class="transport-notes"><p><b>대중교통</b> 예시역 2번 출구에서 도보 5분</p><p><b>주차 안내</b> 예시 주차장 · 하객 2시간 무료</p></div><small class="sample-caption">실제 예식장 정보는 제작 페이지에서 입력해요.</small>`);
  if (sections.accounts) parts.accounts = section('accounts', 'WITH LOVE', '마음 전하실 곳', `<p>함께해 주시는 따뜻한 마음에<br>깊이 감사드립니다.</p><div class="sample-accounts"><details><summary>신랑 측 <span>+</span></summary><p>성우 · 은행명<br><span>계좌번호는 제작 시 입력</span></p></details><details><summary>신부 측 <span>+</span></summary><p>소희 · 은행명<br><span>계좌번호는 제작 시 입력</span></p></details></div>`);
  if (sections.rsvp) parts.rsvp = section('rsvp', 'WILL YOU JOIN US?', '참석 의사 전달', `<p>귀한 걸음을 준비할 수 있도록<br>참석 여부를 알려주세요.</p><div class="rsvp-example"><span>성함</span><span>신랑 측 / 신부 측</span><span>참석 여부 · 동반 인원</span></div><p class="sample-callout">참석 의사 전달하기</p><small class="sample-caption">미리보기에서는 응답을 받지 않아요.</small>`);
  if (sections.guestbook) parts.guestbook = section('guestbook', 'WARM WISHES', '축하의 마음', `<blockquote>두 사람의 모든 계절을 응원해요.<br>오래오래 행복하세요!<cite>소중한 친구 · 예시</cite></blockquote><p class="sample-callout">축하 글 남기기</p><small class="sample-caption">미리보기에서는 글이 저장되지 않아요.</small>`);
  parts.ending = `<footer class="invitation-ending reveal"><p class="ending-script">${selection.templateId === 'cinema' ? 'To be continued.' : 'Together, always.'}</p><p>우리의 시작에 함께해 주셔서 감사합니다.</p><p class="couple-names">${names}</p><span class="small-caps">${WEDDING.middleDots}</span></footer>`;
  const attributes = themeAttributes(selection).replace('class="', 'class="' + (SIGNATURES[selection.templateId] ? `signature-invitation signature-${selection.templateId} ` : ''));
  return `<article ${attributes} aria-label="${e(getTemplate(selection.templateId).name)} 전체 미리보기">${salonBody(selection, parts) || guestBody(selection, parts) || immersiveBody(selection, parts) || editionBody(selection, parts) || signatureBody(selection, parts)}</article>`;
}
