import { PHOTOS, SIGNATURES, getTemplate } from './catalog.mjs?v=20260914-scroll-editions';
import { escapeHtml as e } from './core.mjs?v=20260914-scroll-editions';
import { paperCover } from './special-paper.mjs?v=20260914-scroll-editions';
import { worldsCover } from './special-worlds.mjs?v=20260914-scroll-editions';
import { signatureBody } from './signatures.mjs?v=20260914-scroll-editions';
import { editionCover, editionBody } from './editions.mjs?v=20260914-scroll-editions';
...
export function cover(templateId, thumbnail = false) {
  const special = editionCover(templateId, thumbnail) || paperCover(templateId, thumbnail) || worldsCover(templateId, thumbnail);
...
export function invitation(selection) {
  ...
  return `<article ${attributes} aria-label="${e(getTemplate(selection.templateId).name)} 전체 미리보기">${editionBody(selection, parts) || signatureBody(selection, parts)}</article>`;
}
