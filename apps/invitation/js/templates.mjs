...
const image = (index, classes = '', eager = false) => `<img class="${classes}" src="${e(getPhoto(index).src)}" alt="${e(getPhoto(index).alt)}" ${eager ? 'fetchpriority="high"' : 'loading="lazy"'} decoding="async" width="${getPhoto(index).width}" height="${getPhoto(index).height}">`;
const names = '<span>성우</span><i>&</i><span>소희</span>';

export const cover = (templateId, thumbnail = false) => personalizeCover(renderCover(templateId, thumbnail));
function renderCover(templateId, thumbnail = false) {
