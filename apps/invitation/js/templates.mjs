...
export function cover(templateId, thumbnail = false) {
  ...
  const date = '<p class="cover-date">2027. 03. 06 <span>SATURDAY</span></p>';
...
export function invitation(selection) {
  ...
  const calendar = `<div class="wedding-calendar" aria-label="2027년 3월 달력, 6일 토요일 결혼식"><div class="calendar-week">${['일', '월', '화', '수', '목', '금', '토'].map(day => `<span>${day}</span>`).join('')}</div><div class="calendar-days"><span></span>${Array.from({ length: 31 }, (_, index) => `<span${index === 5 ? ' class="wedding-day" aria-label="6일 결혼식"' : ''}>${index + 1}</span>`).join('')}</div></div>`;
  ...
  html += section('date', 'SAVE OUR DATE', '2027년 3월 6일', `<p class="wedding-time">토요일 오후 2시</p>${calendar}<p class="venue-name">우리의 웨딩홀 · 가든홀</p><small class="sample-caption">시간과 장소는 예시입니다.</small>`);
  ...
  html += `<footer class="invitation-ending reveal"><p class="ending-script">${selection.templateId === 'cinema' ? 'To be continued.' : 'Together, always.'}</p><p>우리의 시작에 함께해 주셔서 감사합니다.</p><p class="couple-names">${names}</p><span class="small-caps">2027 · 03 · 06</span></footer>`;
