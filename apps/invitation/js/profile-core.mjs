...
export const MAX_PHOTOS = 22;
export const MAX_PHOTO_CHARS = 360000;
export const MAX_PHOTO_DIMENSION = 4096;
export const MAX_TRANSACTION_PHOTO_CHARS = 8000000;
export const VENUE_LIMITS = Object.freeze({ name: 80, hall: 80, address: 300, mapUrl: 1000, naverUrl: 1000, kakaoUrl: 1000, transport: 1000, parking: 1000 });
const OPTIONAL_VENUE_FIELDS = ['naverUrl', 'kakaoUrl'];
...
export const emptyProfile = () => ({ schemaVersion: 1, revision: 0, coverId: null, galleryIds: [], mapImageId: null, venue: emptyVenue(), updatedAt: null, updatedBy: null });
...
export function validateVenue(value) {
  ...
  for (const [key, limit] of Object.entries(VENUE_LIMITS)) {
    const text = OPTIONAL_VENUE_FIELDS.includes(key) && !own(value, key) ? '' : value[key];
    if (typeof text !== 'string' || text.length > limit || /[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/.test(text)) throw fail('예식장 안내의 길이와 내용을 확인해주세요.');
    result[key] = text.trim();
  }
  if (!result.name && Object.values(result).some(Boolean)) throw fail('예식장 이름을 먼저 입력해주세요.');
  for (const key of ['mapUrl', ...OPTIONAL_VENUE_FIELDS]) {
    if (!result[key]) continue;
    let url;
    try { url = new URL(result[key]); } catch { throw fail('지도 주소는 https로 시작하는 웹 주소를 입력해주세요.'); }
    if (url.protocol !== 'https:' || !url.hostname || url.username || url.password || /[\s\\]/.test(result[key])) throw fail('지도 주소는 https로 시작하는 웹 주소를 입력해주세요.');
  }
  return result;
}
...
export function validateProfile(value) {
  if (!keysWithin(value, ['schemaVersion', 'revision', 'coverId', 'galleryIds', 'mapImageId', 'venue', 'updatedAt', 'updatedBy'])
      || (own(value, 'schemaVersion') && value.schemaVersion !== 1)
      || (value.coverId !== null && !validPhotoId(value.coverId))
      || (own(value, 'mapImageId') && value.mapImageId !== null && !validPhotoId(value.mapImageId))
  ...
  return { schemaVersion: 1, coverId: value.coverId, galleryIds: [...value.galleryIds], mapImageId: value.mapImageId ?? null, venue: validateVenue(value.venue) };
}
...
export function profilePhotoIds(profile) { return [...new Set([profile.coverId, ...(profile.galleryIds || []), profile.mapImageId].filter(Boolean))]; }
