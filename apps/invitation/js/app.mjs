...
function refreshPreview() {
  const selection = getDraft(route.templateId).selection, previousScroll = window.scrollY, storyPosition = scrollStory.capture();
  scrollStory.dispose();
  dialog('preview-canvas').innerHTML = invitation(selection); mountPreviewEffects(storyPosition);
...
document.addEventListener('click', async event => {
  ...
  try {
    switch (target.dataset.action) {
      ...
      case 'scroll-scenes': {
        const position = scrollStory.capture(); scrollScenes = !scrollScenes; scrollStory.dispose();
        mountPreviewEffects(position); updatePreviewDevice(); break;
      }
