...
- 공개 사이트 배포는 승인 대기다. 자동 승인 검토가 명시적 배포 승인 없이 `master`에 푸시하는 동작을 차단했다. 공동 일정 저장은 완료했지만 정적 항공표·달력·설명은 배포 전이라 이전 내용일 수 있다.

**다음:** 사용자 배포 승인 후 이 변경을 master에 반영하고 Pages 성공·공개 파일을 확인한다.
