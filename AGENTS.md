...
### 2026-09-13 — 여행 이동 국가·예산 연동·출발 준비, 로컬 검증 완료
...
- `npm run build`와 `npm run check`의 1,422개 검사·배포 참조·문법 검사를 통과했다. 테스트 대역 1440px/390px에서 여덟 화면·이동 국가·비용 6항목, Travel↔WeCost 금액·지급·이력·동시 편집·실패 보존·오프라인을 확인했고 운영 DB 요청은 0이었다. 운영 저장·원격 배포 결과는 아직 기록하지 않는다.

**다음:** 배포 후 공개 파일과 Travel·WeCost 연결 화면을 읽기 위주로 확인하고 결과를 기록한다.
