...
import { normalizeTransaction, buildMarketSummary } from '../../../shared/homehunt/market-core.mjs';
import { configPath, publicDataPath } from '../paths.mjs';

const regionConfig = JSON.parse(await readFile(configPath('regions.json'), 'utf8'));
const trackedConfig = JSON.parse(await readFile(configPath('tracked-apartments.json'), 'utf8'));
...
await writeFile(publicDataPath('market-summary.json'), `${JSON.stringify(summary)}\n`, 'utf8');
...
await writeFile(publicDataPath('apartment-history.json'), `${JSON.stringify({ version: 2, source: '국토부 아파트 실거래', generatedAt, coveredMonths: lookback, ...historyRange, apartments })}\n`, 'utf8');
