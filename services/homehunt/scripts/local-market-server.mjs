...
import { publicDataPath, sourceDataPath, localStatePath } from '../paths.mjs';
import {
  filterCatalogForRecommendation,
  normalizeRecommendationFilters,
} from '../../../shared/homehunt/recommendation-core.mjs';
import {
  KakaoDailyLedger,
...
import { normalizeGeoPoint } from '../../../shared/homehunt/transport-core.mjs';
...
const {
  ...
  resolveHistoryRange,
} = require('../server/molit.cjs');

const CACHE_DIR = localStatePath('market-cache');
...
const PUBLIC_SUPPLY_FILE = publicDataPath('home-supply.json');
const TMAP_LEDGER_FILE = localStatePath('tmap-transit-usage.json');
const KAKAO_LEDGER_FILE = localStatePath('kakao-transit-usage.json');
...
async function loadCatalog() {
  if (!catalogPromise) {
    catalogPromise = fs.readFile(publicDataPath('apartment-catalog-seoul-gyeonggi.json'), 'utf8')
      .catch(() => fs.readFile(sourceDataPath('apartment-catalog.json'), 'utf8'))
