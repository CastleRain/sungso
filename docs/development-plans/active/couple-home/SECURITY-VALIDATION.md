...
3. 신규 서버 검색 키 설정·재조회·Render Live는 [운영 근거](OPERATIONS.md)의 후속 확인에서 완료했다. 이전 키는 재발급 후에도 일정 기간 유효하므로 즉시 폐기는 별도로 확인한다. 소스에서 브라우저 키를 없애거나 새 키를 설정한 사실을 이전 자격증명 폐기 완료로 기록하지 않는다.
