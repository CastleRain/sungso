...
로그아웃 실패 검사는 제어판의 `로그아웃 실패 켜기` 뒤 실제 앱 로그아웃 버튼으로 실행한다. 기본 SDK의 signOut만 거절하며 개인 화면 제거·새로고침 후 잠금·직접 링크 차단을 확인한다. `로그아웃 실패 끄기`와 `로그아웃 다시 시도`로 복구한다. 이 실패 설정도 sessionStorage의 QA 전용 키이며 운영 환경에 배포되지 않는다.
