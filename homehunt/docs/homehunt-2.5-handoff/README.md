...
이 폴더는 HomeHunt를 처음 보는 사람이나 다음 고도화 작업자가 현재 구현을 빠르게 이해하기 위한 기준 문서다. 화면 모양만 설명하지 않고, 어떤 데이터가 어디서 오며 어떤 조건으로 후보·평균가격·통근·예측·분양 알림이 만들어지는지 함께 기록한다.
...
| 회사·건물명 검색 | NAVER API HUB 지역 검색 인식 |
...
그다음 `http://127.0.0.1:8000/homehunt/`를 연다. 화면에 `서버 재시작 필요`가 나오면 실행 중인 로컬 API가 `2.5.0`보다 오래된 것이다.
