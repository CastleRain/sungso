...
화면 버전은 **4.12.1**, 로컬 API 계약은 **2.9.0**입니다. GitHub Pages는 `master`의 정적 화면을 배포하며 Google 로그인과 Firestore 개인 백업을 사용합니다. 가격·통근·시설 조회는 로컬 검색 서버를 사용하고, 공개 API는 아직 비활성 상태입니다. 최신 화면 배포만으로 검색 서버가 활성화되지는 않습니다. [클라우드 저장·Firebase/Vercel 배포](docs/firebase-cloud.md)에 현재 범위와 실행 방법을 정리했습니다.
