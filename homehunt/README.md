...
현재 작업 트리의 화면 버전은 **4.4.0**, 로컬 API 계약은 **2.6.3**입니다. 화면과 서버 버전은 별도로 관리합니다. 4.4.0은 Google 로그인과 무료 Firestore 개인 기록 저장을 추가합니다. 검색·통근 서버의 클라우드 코드는 별도 코드베이스로 준비했지만 Firebase Spark 플랜에서는 배포할 수 없어 공개 API는 아직 비활성 상태입니다. [클라우드 저장·Firebase/Vercel 배포](docs/firebase-cloud.md)에 현재 범위와 실행 방법을 정리했습니다.
...
UI 4.3.3은 Kakao 단독 통근 확인과 최대 10곳·30개 경로의 순차 확인을 지원합니다. 가격 검색은 경로를 호출하지 않고 현재 화면에서 확인한 후보는 묶음 조회에서 건너뜁니다. Kakao 경로는 저장·다음 검색 재사용 없이 현재 화면에서만 표시합니다.
