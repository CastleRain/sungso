...
현재 작업 트리의 화면 버전은 **4.12.0**, 로컬 API 계약은 **2.9.0**입니다. 4.12.0은 로컬 검증 버전이며 공개 Pages는 4.4.0을 유지합니다. Google 로그인과 Firestore 개인 백업은 기존 기능을 사용합니다. 검색·통근 서버의 클라우드 코드는 별도 코드베이스로 준비했지만 공개 API는 아직 비활성 상태입니다. [클라우드 저장·Firebase/Vercel 배포](docs/firebase-cloud.md)에 현재 범위와 실행 방법을 정리했습니다.
